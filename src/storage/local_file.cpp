...
[[nodiscard]] std::filesystem::path TemporaryPathFor(
    const std::filesystem::path& destination, std::uint64_t suffix) {
...
[[nodiscard]] std::error_code CommitTemporaryFile(
...
WriteLocalFileResult AtomicFileTransaction::Commit(void) {
  ...
  if (error) {
    const auto kind = existing_file_policy_ == ExistingFilePolicy::kPreserve &&
...
const std::filesystem::path& AtomicFileTransaction::temporary_path(
    void) const noexcept {
  return temporary_;
}
...
PrepareAtomicFileResult PrepareAtomicFile(
    const std::filesystem::path& path,
    ExistingFilePolicy existing_file_policy) {
  std::error_code error;
  ...
  if (!temporary_path.has_value()) {
    return std::unexpected(Failure(LocalFileFailureKind::kTemporaryPathFailed,
...
WriteLocalFileResult WriteLocalFileAtomically(
