...
class AtomicFileTransaction final {
 public:
  ...
  [[nodiscard]] const std::filesystem::path& temporary_path(
      void) const noexcept;

 private:
  friend PrepareAtomicFileResult PrepareAtomicFile(const std::filesystem::path&,
                                                   ExistingFilePolicy);
...
[[nodiscard]] ReadLocalFileResult ReadLocalFile(
    const std::filesystem::path& path);

/// Reserves a temporary sibling for caller-controlled output generation.
[[nodiscard]] PrepareAtomicFileResult PrepareAtomicFile(
    const std::filesystem::path& path, ExistingFilePolicy existing_file_policy);

/// Writes \p content to a temporary sibling and atomically commits it.
[[nodiscard]] WriteLocalFileResult WriteLocalFileAtomically(
