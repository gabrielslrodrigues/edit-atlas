...
[[nodiscard]] std::optional<FormatRegistrationError> ValidateDescriptor(
...
[[nodiscard]] std::string NormalizeExtension(std::string_view extension) {
  ...
  std::ranges::transform(extension, std::back_inserter(normalized), LowerAscii);
  return normalized;
}
...
std::expected<void, FormatRegistrationError> FormatRegistry::RegisterImporter(
...
std::expected<void, FormatRegistrationError> FormatRegistry::RegisterExporter(
...
const Importer* FormatRegistry::FindImporter(
...
const Exporter* FormatRegistry::FindExporter(
    std::string_view identifier) const noexcept {
  const auto match = exporters_.find(identifier);
  return match == exporters_.end() ? nullptr : match->second.get();
}

std::vector<const Importer*> FormatRegistry::ImportersForExtension(
...
std::vector<const Exporter*> FormatRegistry::ExportersForExtension(
    std::string_view extension) const {
...
std::vector<const Importer*> FormatRegistry::importers(void) const {
  ...
  std::ranges::transform(importers_, std::back_inserter(result),
