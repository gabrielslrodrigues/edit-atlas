...
[[nodiscard]] std::int64_t NominalFramesPerSecond(
    const FrameRate& rate) noexcept {
...
[[nodiscard]] std::int64_t DroppedFramesPerMinute(
    const FrameRate& rate) noexcept {
...
void AppendTwoDigits(std::string& output, std::int64_t value) {
...
std::expected<FrameRate, FrameRateError> FrameRate::Create(
...
std::int32_t FrameRate::numerator(void) const noexcept { return numerator_; }

std::int32_t FrameRate::denominator(void) const noexcept {
  return denominator_;
}

std::expected<Timecode, TimecodeError> Timecode::Create(
    std::int64_t hours, std::int64_t minutes, std::int64_t seconds,
...
std::expected<Timecode, TimecodeError> Timecode::FromFrameCount(
    std::int64_t frame_count, FrameRate rate, TimecodeMode mode) noexcept {
  const auto nominal_rate = NominalFramesPerSecond(rate);
  if (nominal_rate <= 0 ||
      nominal_rate > std::numeric_limits<std::int32_t>::max()) {
    return std::unexpected(TimecodeError::kUnsupportedFrameRate);
  ...
  if (mode == TimecodeMode::kDropFrame) {
    ...
    if (remaining_frames > dropped_frames) {
      labeled_frame += dropped_frames * ((remaining_frames - dropped_frames) /
                                         frames_per_minute);
    }
  }
...
std::int32_t Timecode::hours(void) const noexcept { return hours_; }

std::int32_t Timecode::minutes(void) const noexcept { return minutes_; }

std::int32_t Timecode::seconds(void) const noexcept { return seconds_; }

std::int32_t Timecode::frames(void) const noexcept { return frames_; }

const FrameRate& Timecode::rate(void) const noexcept { return rate_; }

TimecodeMode Timecode::mode(void) const noexcept { return mode_; }
...
std::expected<TimecodeRange, TimecodeRangeError> TimecodeRange::Create(
    Timecode start, Timecode end_exclusive) noexcept {
...
const Timecode& TimecodeRange::start(void) const noexcept { return start_; }
...
std::string TimecodeRange::Duration(void) const {
  ...
  const auto nominal_rate = NominalFramesPerSecond(rate);
  auto labeled_frame = DurationInFrames();
  ...
  const auto frames_per_hour =
      nominal_rate * kMinutesPerHour * kSecondsPerMinute;
  const auto frames_per_minute = nominal_rate * kSecondsPerMinute;
  const auto hours = labeled_frame / frames_per_hour;
  labeled_frame %= frames_per_hour;
  const auto minutes = labeled_frame / frames_per_minute;
