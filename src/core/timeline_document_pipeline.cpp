...
[[nodiscard]] std::string_view ExtensionFromRequest(
...
[[nodiscard]] const Importer* BreakProbeTieWithExtension(
    const std::vector<ProbeResult>& matches,
    const std::vector<const Importer*>& by_extension) {
  const Importer* selected = nullptr;
  for (const auto& match : matches) {
    if (std::ranges::find(by_extension, match.importer) == by_extension.end()) {
      continue;
    }
    if (selected != nullptr) {
      return nullptr;
...
[[nodiscard]] const Importer* SelectImporter(
    const FormatRegistry& registry, const ImportRequest& request,
  ...
  for (const auto* importer : registry.importers()) {
    try {
      ...
      diagnostics.emplace_back(
          MakeDiagnostic(DiagnosticSeverity::kWarning,
...
ImportResult TimelineDocumentPipeline::Import(
  ...
  if (format_identifier.has_value()) {
    ...
    if (importer == nullptr) {
      return FailedImport({}, pipeline_diagnostic_code::kUnknownImportFormat,
    ...
    importer =
        SelectImporter(registry_, request, discovery_diagnostics, ambiguous);
    if (importer == nullptr) {
      const auto code = ambiguous
                            ? pipeline_diagnostic_code::kAmbiguousImportFormat
      ...
      return FailedImport(std::move(discovery_diagnostics), code, message);
    }
  }
  ...
  try {
    ...
    if (!result.document.has_value() && !HasError(result.diagnostics)) {
      result.diagnostics.emplace_back(
          MakeDiagnostic(DiagnosticSeverity::kError,
    ...
    return FailedImport(
        std::move(discovery_diagnostics),
        pipeline_diagnostic_code::kImportException,
        "Importer '" + importer->descriptor().identifier + "' failed");
  }
}

ExportResult TimelineDocumentPipeline::Export(
    const ExportRequest& request, std::string_view format_identifier) const {
  ...
  try {
    ...
    return FailedExport(
        ...
        "Exporter '" + exporter->descriptor().identifier + "' failed");
  }
}
