...
std::expected<std::vector<std::byte>, std::string> EncodeRgbImageAsPng(
    const core::RgbImage& image, std::int32_t maximum_width,
    std::int32_t maximum_height) {
  ...
  if (png_image_write_to_memory(&png, nullptr, &output_size, 0,
                                ...
                                nullptr) == 0) {
    auto message =
        std::string{"Could not determine the encoded PNG size: "} + png.message;
    png_image_free(&png);
  ...
  if (png_image_write_to_memory(&png, output.data(), &output_size, 0,
                                ...
                                nullptr) == 0) {
    auto message =
        std::string{"Could not encode the RGB image as PNG: "} + png.message;
    png_image_free(&png);
    return std::unexpected(std::move(message));
  }
  png_image_free(&png);
