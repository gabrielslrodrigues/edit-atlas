...
[[nodiscard]] bool HasInitialFrame(
    std::span<const core::TimelineEventField> projection) noexcept {
  ...
  for (const auto& event_image : request.event_images) {
    ...
    if (!encoded.has_value()) {
      return std::unexpected(
          ErrorDiagnostic(diagnostic_code::kImageWriteFailed,
...
[[nodiscard]] std::string_view TrackKindText(
    core::TrackKind kind, const WorkbookText& text) noexcept {
  switch (kind) {
...
[[nodiscard]] std::string_view DiagnosticSeverityText(
    core::DiagnosticSeverity severity, const WorkbookText& text) noexcept {
  switch (severity) {
...
[[nodiscard]] std::string MetadataText(
    const std::vector<core::MetadataEntry>& metadata, std::string_view key) {
...
[[nodiscard]] std::string CommentText(
...
[[nodiscard]] bool BooleanOption(
    const std::vector<core::MetadataEntry>& options, std::string_view key,
    bool default_value) {
  const auto option =
  ...
  if (const auto* value = std::get_if<bool>(&option->value); value != nullptr) {
    return *value;
  }
  return default_value;
}

[[nodiscard]] WorkbookLanguage LanguageOption(
    const std::vector<core::MetadataEntry>& options) {
...
void WriteNumber(WriteStatus& status, lxw_worksheet* worksheet, lxw_row_t row,
  ...
  status.Record(worksheet_write_number(worksheet, row, column, value, format));
}
...
void WriteInitialFrame(WriteStatus& status, lxw_worksheet* worksheet,
  ...
  options.x_scale = 1.0;
  options.y_scale = 1.0;
  options.object_position = static_cast<std::uint8_t>(LXW_OBJECT_MOVE_AND_SIZE);
...
void WriteEventsSheet(WriteStatus& status, lxw_worksheet* worksheet,
  ...
  for (std::size_t index = 0; index < document.events.size(); ++index) {
    ...
    if (!initial_frames.empty()) {
      status.Record(worksheet_set_row_pixels(worksheet, row,
                                             kInitialFrameRowHeight, nullptr));
    }
    for (std::size_t column = 0; column < projection.size(); ++column) {
      if (projection[column] == core::TimelineEventField::kInitialFrame) {
        WriteInitialFrame(status, worksheet, row,
                          static_cast<lxw_col_t>(column), initial_frames[index],
                          event);
        continue;
      }
      WriteEventField(status, worksheet, row, static_cast<lxw_col_t>(column),
                      projection[column], event, text, wrapped_format);
    }
  }
}
...
void WriteDiagnosticsSheet(WriteStatus& status, lxw_worksheet* worksheet,
                           ...
                           const WorkbookText& text, lxw_format* header_format,
                           lxw_format* wrapped_format) {
  WriteHeader(status, worksheet, text.DiagnosticColumns(), text, header_format);
  worksheet_freeze_panes(worksheet, 1, 0);
  ...
  for (std::size_t index = 0; index < document.diagnostics.size(); ++index) {
    ...
    WriteString(status, worksheet, row, 2, diagnostic.message, wrapped_format);
  ...
  if (!document.diagnostics.empty()) {
    status.Record(worksheet_autofilter(
        worksheet, 0, 0, static_cast<lxw_row_t>(document.diagnostics.size()),
        5));
  }
}
...
XlsxExporter::XlsxExporter(void)
    : descriptor_{
...
core::ExportResult XlsxExporter::Export(
  ...
  if (timeline != nullptr) {
    WriteTimelineSheet(status, timeline, request.document, text, header_format);
  }
