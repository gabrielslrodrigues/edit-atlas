...
[[nodiscard]] std::optional<std::string> DecodeUtf16(
    std::span<const std::byte> input, bool little_endian) {
  if (input.size() % 2 != 0) {
    return std::nullopt;
  }
  ...
  for (std::size_t index = 0; index < input.size(); index += 2) {
    ...
    if (unit >= 0xD800 && unit <= 0xDBFF) {
      ...
      code_point =
          0x10000 +
          ((static_cast<std::uint32_t>(unit - 0xD800) << 10) | (low - 0xDC00));
      index += 2;
...
[[nodiscard]] std::string DecodeWindows1252(std::span<const std::byte> input) {
  ...
  for (const auto byte : input) {
    ...
    if (value >= 0x80 && value <= 0x9F) {
      code_point = kControlCodePoints[static_cast<std::size_t>(value - 0x80)];
    }
    AppendUtf8(output, code_point);
  }
  return output;
...
[[nodiscard]] std::optional<DecodedText> DecodeText(
...
[[nodiscard]] std::optional<FrameRate> ParseFrameRate(
  ...
  for (const auto& option : request.options) {
    ...
    if (const auto* integer = std::get_if<std::int64_t>(&option.value)) {
      ...
      return rate.has_value() ? std::optional<FrameRate>{*rate} : std::nullopt;
    }
...
[[nodiscard]] bool HasFrameRateOption(const ImportRequest& request) {
  return std::ranges::any_of(request.options, [](const MetadataEntry& option) {
...
Cmx3600Importer::Cmx3600Importer(void)
    : descriptor_{
...
core::ProbeConfidence Cmx3600Importer::Probe(
    const core::ImportRequest& request) const {
...
core::ImportResult Cmx3600Importer::Import(
    const core::ImportRequest& request) const {
  ...
  if (!rate.has_value()) {
    AddDiagnostic(diagnostics, DiagnosticSeverity::kError,
                  has_frame_rate_option ? diagnostic_code::kInvalidFrameRate
  ...
  if (found_fcm) {
    document.metadata.emplace_back(
        "cmx3600.fcm",
        std::string{mode == TimecodeMode::kDropFrame ? "drop" : "non-drop"});
  }
  ...
  for (std::size_t index = 0; index < lines.size(); ++index) {
    ...
    if (trimmed.starts_with('*')) {
      if (last_event != nullptr) {
        ...
        document.metadata.emplace_back("cmx3600.comment",
                                       std::string{Trim(trimmed.substr(1))});
      }
      continue;
    }
    if (trimmed.starts_with("M2 ")) {
      if (last_event == nullptr) {
        AddDiagnostic(diagnostics, DiagnosticSeverity::kWarning,
                      ...
                      "Motion-effect record has no preceding event", request,
                      line_number);
      } else {
    ...
    if (tokens.empty() || !IsEventNumber(tokens.front())) {
      ...
      AddDiagnostic(diagnostics, DiagnosticSeverity::kWarning,
                    ...
                    "Unrecognized CMX 3600 content was preserved", request,
                    line_number);
      continue;
    }
    if (tokens.size() < 8) {
      AddDiagnostic(diagnostics, DiagnosticSeverity::kError,
                    ...
                    "Event record has too few fields", request, line_number);
      continue;
    }

    const auto timecode_offset = tokens.size() - 4;
    std::array<std::optional<Timecode>, 4> timecodes;
    for (std::size_t timecode_index = 0; timecode_index < 4; ++timecode_index) {
      timecodes[timecode_index] =
          ParseTimecode(tokens[timecode_offset + timecode_index], *rate, mode);
    }
    ...
    if (!source_range.has_value() || !record_range.has_value()) {
      AddDiagnostic(diagnostics, DiagnosticSeverity::kError,
                    diagnostic_code::kInvalidTimecode,
                    "Event record contains a reversed timecode range", request,
                    line_number);
      continue;
    }
    ...
    if (edit_type == core::EditType::kOther) {
      ...
      AddDiagnostic(diagnostics, DiagnosticSeverity::kWarning,
                    diagnostic_code::kUnknownContent,
                    "Unknown edit type was preserved", request, line_number);
    }
  }
