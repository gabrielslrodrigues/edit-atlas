...
class VideoDecoder {
 public:
  ...
  [[nodiscard]] virtual const VideoMediaInfo& Information(
  ...
  [[nodiscard]] virtual std::expected<void, VideoDecoderFailure> Seek(
      std::int64_t timestamp) = 0;

  /// Seeks backward to a usable point at or before a zero-based frame index.
  [[nodiscard]] virtual std::expected<void, VideoDecoderFailure> SeekToFrame(
