...
[[nodiscard]] std::optional<std::int64_t> OptionalTimestamp(
    std::int64_t value) {
  return value == AV_NOPTS_VALUE ? std::nullopt
                                 : std::optional<std::int64_t>{value};
}

[[nodiscard]] std::vector<MediaMetadataEntry> MetadataEntries(
...
class FfmpegVideoDecoder final : public VideoDecoder {
 public:
  ...
  [[nodiscard]] const VideoMediaInfo& Information(
      void) const noexcept override {
    return information_;
  }

  [[nodiscard]] std::expected<void, VideoDecoderFailure> Seek(
      std::int64_t timestamp) override {
    const auto result = av_seek_frame(format_context_->Get(),
                                      static_cast<int>(selected_video_stream_),
                                      timestamp, AVSEEK_FLAG_BACKWARD);
    if (result < 0) {
      return std::unexpected(
          MakeFailure(path_, VideoDecoderFailureKind::kSeek, result,
  ...
  [[nodiscard]] std::expected<void, VideoDecoderFailure> SeekToFrame(
      std::int64_t frame_index) override {
    const auto* stream =
    ...
    if (frame_index < 0 || stream->avg_frame_rate.num <= 0 ||
      ...
      return std::unexpected(
          MakeFailure(path_, VideoDecoderFailureKind::kSeek, 0,
                      information_.streams[selected_video_stream_].codec_name,
  ...
  ReadFrame(const VideoFrameOutputOptions& options) override {
    ...
    while (true) {
      ...
      if (receive_result != AVERROR(EAGAIN)) {
        return std::unexpected(MakeFailure(
            path_, VideoDecoderFailureKind::kDecodeFrame, receive_result,
            information_.streams[selected_video_stream_].codec_name,
 ...
 private:
  [[nodiscard]] std::expected<bool, VideoDecoderFailure> ReadVideoPacket(void) {
    while (true) {
      av_packet_unref(packet_.get());
      ...
      if (packet_->stream_index != static_cast<int>(selected_video_stream_)) {
        continue;
      }
  ...
  ResolveOutputDimensions(const VideoFrameOutputOptions& options) const {
    if (frame_->width <= 0 || frame_->height <= 0 ||
      ...
      return std::unexpected(
          MakeFailure(path_, VideoDecoderFailureKind::kConvertFrame, 0,
                      information_.streams[selected_video_stream_].codec_name,
    ...
    if (options.size_limit.has_value()) {
      ...
      if (size_limit.maximum_width <= 0 || size_limit.maximum_height <= 0) {
        return std::unexpected(
            MakeFailure(path_, VideoDecoderFailureKind::kConvertFrame, 0,
                        information_.streams[selected_video_stream_].codec_name,
      ...
      if (output_width > size_limit.maximum_width ||
        ...
        const auto width_limited =
            static_cast<std::int64_t>(size_limit.maximum_width) *
                output_height <=
            static_cast<std::int64_t>(size_limit.maximum_height) * output_width;
        if (width_limited) {
          output_height = std::max(
              1, static_cast<int>(static_cast<std::int64_t>(frame_->height) *
                                  size_limit.maximum_width / frame_->width));
          output_width = size_limit.maximum_width;
        } else {
          output_width = std::max(
              1, static_cast<int>(static_cast<std::int64_t>(frame_->width) *
  ...
  [[nodiscard]] std::expected<core::RgbImage, VideoDecoderFailure> ConvertImage(
      const VideoFrameOutputOptions& options) {
    auto dimensions = ResolveOutputDimensions(options);
    if (!dimensions.has_value()) {
      return std::unexpected(std::move(dimensions.error()));
    }

    const auto row_stride = static_cast<std::size_t>(dimensions->width) * 3U;
    ...
    auto* scale_context = sws_getCachedContext(
        ...
        dimensions->height, AV_PIX_FMT_RGB24, SWS_BILINEAR, nullptr, nullptr,
        nullptr);
    ...
    std::array<std::uint8_t*, 4> destination_planes{destination, nullptr,
                                                    nullptr, nullptr};
    std::array<int, 4> destination_strides{static_cast<int>(row_stride), 0, 0,
                                           0};
  ...
  [[nodiscard]] std::optional<std::int64_t> FrameIndex(
    ...
    const auto stream_start = stream->start_time == AV_NOPTS_VALUE
                                  ...
                                  : stream->start_time;
    if ((stream_start > 0 &&
         timestamp < std::numeric_limits<std::int64_t>::min() + stream_start) ||
        (stream_start < 0 &&
         timestamp > std::numeric_limits<std::int64_t>::max() + stream_start)) {
      return std::nullopt;
    }
    ...
    const auto frame_index = av_rescale_q(timestamp_offset, stream->time_base,
...
VideoDecoder::Open(const std::filesystem::path& path) {
  ...
  if (stream_result < 0) {
    return std::unexpected(MakeFailure(
        path, VideoDecoderFailureKind::kReadStreamInformation, stream_result,
        {}, "could not read media stream information"));
  }
  ...
  if (best_stream < 0) {
    return std::unexpected(
        MakeFailure(path, VideoDecoderFailureKind::kNoVideoStream, best_stream,
  ...
  if (decoder_result < 0) {
    return std::unexpected(
        MakeFailure(path, VideoDecoderFailureKind::kOpenDecoder, decoder_result,
