...
[[nodiscard]] QString SuggestedDirectory(void) {
  ...
  const auto configured_directory =
      settings.value(QStringLiteral("supportBundle/lastDirectory")).toString();
...
QString SupportBundleViewModel::ResultPath(void) const { return result_path_; }

qulonglong SupportBundleViewModel::IncludedLogFileCount(void) const noexcept {
  return included_log_file_count_;
}

QString SupportBundleViewModel::ErrorText(void) const { return error_text_; }
...
bool SupportBundleViewModel::Start(const QUrl& destination,
  ...
  if (!view_model_.Export(destination_path, replace_existing).has_value()) {
    SetImmediateFailure(tr("The support-bundle export could not be started."));
    return false;
  }
  return true;
}
...
void SupportBundleViewModel::handleFinished(void) {
  ...
  switch (failure.kind) {
    ...
    case support::SupportBundleFailureKind::kCommitFailed:
      error_text_ =
          tr("The completed support bundle could not replace the "
             "destination file.");
      break;
  }
