...
[[nodiscard]] QStringList FormatIdentifiers(
...
ApplicationInformationViewModel::ApplicationInformationViewModel(
    ...
    : QObject{parent} {
  const auto environment = presentation::CreateDiagnosticEnvironment(registry);
...
bool ApplicationInformationViewModel::OpenProjectWebsite(void) const {
  return presentation::desktop_integration::OpenExternalUrl(
      QUrl{QStringLiteral("https://github.com/gabrielslrodrigues/edit-atlas")});
}

bool ApplicationInformationViewModel::OpenQtLicensingInformation(void) const {
  return presentation::desktop_integration::OpenExternalUrl(QUrl{QStringLiteral(
