...
class AccessibleComboBox final : public QAccessibleWidget,
  ...
  public:
    ...
    int
    indexOfChild(const QAccessibleInterface *child_interface) const override {
...
class AccessibleProjectionList final : public QAccessibleWidget,
  ...
  public:
    ...
    QAccessibleInterface *child(int index) const override {
        ...
        const auto match =
            std::ranges::find_if(children_, [item](const auto *candidate) {
    ...
    indexOfChild(const QAccessibleInterface *child_interface) const override {
        ...
        return item_interface == nullptr ? -1
...
bool AccessibleProjectionItem::isValid(void) const {
    return parent_ != nullptr && parent_->list() != nullptr &&
           item_ != nullptr && parent_->list()->row(item_) >= 0;
}
...
QAccessible::State AccessibleProjectionItem::state(void) const {
    ...
    const auto visible =
        list->isVisible() &&
        list->viewport()->rect().intersects(list->visualItemRect(item_));
