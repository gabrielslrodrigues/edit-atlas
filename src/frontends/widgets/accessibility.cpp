...
class AccessibleComboBox final : public QAccessibleWidget,
 ...
 public:
  ...
  QAccessibleInterface* child(int index) const override {
    if (index == 0) {
      return QAccessible::queryAccessibleInterface(combo_box()->view());
    }
    if (index == 1 && combo_box()->isEditable()) {
      return QAccessible::queryAccessibleInterface(combo_box()->lineEdit());
    }
    return nullptr;
  }

  int childCount(void) const override {
    return combo_box()->isEditable() ? 2 : 1;
  }
  ...
  int indexOfChild(const QAccessibleInterface* child_interface) const override {
    ...
    if (child_interface->object() == combo_box()->view()) {
      return 0;
    }
    if (combo_box()->isEditable() &&
        child_interface->object() == combo_box()->lineEdit()) {
      return 1;
    }
    return -1;
  }
  ...
  QAccessibleInterface* focusChild(void) const override {
    return combo_box()->isEditable() ? child(1) : nullptr;
  }

  QString text(QAccessible::Text type) const override {
    if (type == QAccessible::Name || type == QAccessible::Value) {
      return combo_box()->isEditable() ? combo_box()->lineEdit()->text()
                                       : combo_box()->currentText();
    }
    if (type == QAccessible::Accelerator) {
      return QKeySequence{Qt::Key_Down}.toString(QKeySequence::NativeText);
    }
  ...
  QAccessible::State state(void) const override {
    ...
    accessible_state.expanded = combo_box()->view()->isVisible();
    accessible_state.editable = combo_box()->isEditable();
  ...
  void setCurrentValue(const QVariant& value) override {
    ...
    if (!converted || requested < 0.0 ||
        requested >= static_cast<double>(combo_box()->count())) {
      return;
    }
  ...
  QVariant maximumValue(void) const override {
    return combo_box()->count() > 0 ? combo_box()->count() - 1 : 0;
  }
 ...
 private:
  QComboBox* combo_box(void) const {
 ...
 private:
  ...
  QAction* FindAction(const QString& identifier) const {
    ...
    for (auto* action : button()->menu()->actions()) {
      if (action->property("accessibleIdentifier").toString() == identifier) {
        return action;
      }
    }
    return nullptr;
...
class AccessibleProjectionItem final : public QAccessibleInterface,
 ...
 public:
  ...
  QAccessible::Role role(void) const override { return QAccessible::ListItem; }
  QAccessible::State state(void) const override;
  void* interface_cast(QAccessible::InterfaceType type) override {
 ...
 public:
  ...
  ~AccessibleProjectionList(void) override {
    ...
    for (auto* child : children_) {
      QAccessible::deleteAccessibleInterface(QAccessible::uniqueId(child));
    }
  }
  ...
  int indexOfChild(const QAccessibleInterface* child_interface) const override {
    ...
    return item_interface == nullptr ? -1 : list()->row(item_interface->item());
  }
  ...
  QListWidget* list(void) const { return qobject_cast<QListWidget*>(object()); }
...
QAccessible::State AccessibleProjectionItem::state(void) const {
  ...
  accessible_state.selectable =
      static_cast<bool>(item_->flags() & Qt::ItemIsSelectable);
