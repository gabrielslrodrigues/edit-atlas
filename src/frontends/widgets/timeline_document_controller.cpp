...
[[nodiscard]] bool HasDiagnosticCode(
    const std::vector<core::Diagnostic>& diagnostics, std::string_view code) {
...
TimelineDocumentController::TimelineDocumentController(
  ...
  connect(&view_model_,
          &presentation::TimelineDocumentViewModel::documentStateChanged, this,
          &TimelineDocumentController::handleDocumentStateChanged);
  connect(&view_model_,
          &presentation::TimelineDocumentViewModel::exportFinished, this,
          &TimelineDocumentController::handleExportFinished);
  connect(&view_model_, &presentation::TimelineDocumentViewModel::filterChanged,
          this, &TimelineDocumentController::HandleFilterChanged);
  connect(
      &view_model_,
      &presentation::TimelineDocumentViewModel::frameExtractionProgressChanged,
      this, [this](qulonglong completed, qulonglong total) {
        ...
        export_progress_->setLabelText(tr("Extracting initial frames: %1 of %2")
                                           .arg(completed)
                                           .arg(total));
      });
...
void TimelineDocumentController::ShowSpreadsheetExporterUnavailable(void) {
  QMessageBox message{
  ...
  SetAutomationIdentifier(message, u"spreadsheetExporterUnavailableDialog");
  SetAutomationIdentifier(*close, u"closeDialogButton");

  message.exec();
}

bool TimelineDocumentController::ConfirmSpreadsheetOverwrite(
  ...
  SetAutomationIdentifier(*cancel_button, u"cancelReplaceSpreadsheetButton");

  confirmation.exec();
  return confirmation.clickedButton() == replace_button;
}

void TimelineDocumentController::ShowSpreadsheetExportProgress(void) {
...
void TimelineDocumentController::exportSpreadsheet(void) {
  ...
  if (exporters.empty()) {
    ShowSpreadsheetExporterUnavailable();
    return;
  }
  ...
  if (!ConfirmSpreadsheetOverwrite(destination)) {
    return;
  }
...
void TimelineDocumentController::OpenTimeline(void) {
  ...
  dialog.setNameFilters(
      {tr("Supported timeline files (%1)")
           .arg(patterns.isEmpty() ? QStringLiteral("*") : patterns.join(u' ')),
       tr("All files (*)")});
  dialog.resize(1000, 650);
  if (dialog.exec() == QDialog::Accepted) {
    const auto selected_files = dialog.selectedFiles();
...
void TimelineDocumentController::handleExportFinished(void) {
  ...
  if (!result->has_value()) {
    if (!HasDiagnosticCode(
            ...
            services::timeline_frame_extraction_diagnostic_code::kCancelled)) {
    ...
    informative_text.emplace_back(
        tr("Embedded initial-frame images from %1 (%2, %3×%4, %5/%6 "
           "fps, starting at %7); decoded %8 unique frame(s).")
            .arg(
                QFileInfo{PathText(receipt.video_information->path)}.fileName())
            ...
            .arg(receipt.video_information
                     ->streams[receipt.video_information->selected_video_stream]
                     .height)
            .arg(receipt.video_mapping->video_start_timecode.rate().numerator())
  ...
  auto* close_button = message.addButton(tr("Close"), QMessageBox::RejectRole);
...
void TimelineDocumentController::handleDocumentStateChanged(void) {
  ...
  if (view_model_.DocumentState() ==
      ...
      HasDiagnosticCode(view_model_.ImportFailure()->diagnostics,
    ...
    if (auto* selector = dialog.findChild<QComboBox*>(); selector != nullptr) {
      SetAutomationIdentifier(*selector, u"frameRateSelector");
    }
    SetInputDialogButtonAutomationIdentifiers(dialog, u"acceptFrameRateButton",
                                              u"cancelFrameRateButton");
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      if (index >= 0) {
        StartImport(PathText(view_model_.ImportFailure()->path),
...
void TimelineDocumentController::ShowRenderedVideoExportFailure(
  ...
  switch (failure.kind) {
    case services::TimelineRenderedVideoExportFailureKind::kVideoRequired:
      description =
          tr("Select a matching rendered video for the Initial "
             "Frame column.");
      break;
