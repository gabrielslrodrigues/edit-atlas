...
void ShowWarning(QWidget& window, const QString& title, const QString& text,
  ...
  QMessageBox message{QMessageBox::Warning, title, text, QMessageBox::NoButton,
                      &window};
...
void TimelineTemplateController::duplicateTemplate(void) {
  ...
  const auto name =
      PromptForTemplateName(tr("Duplicate Template"), tr("Name for the copy:"),
...
void TimelineTemplateController::LoadTemplates(void) {
  ...
  for (const auto& diagnostic : *result) {
    details.push_back(QStringLiteral("%1: %2").arg(PathText(diagnostic.path),
                                                   Utf8(diagnostic.message)));
    SPDLOG_WARN("Skipped timeline template {}: {}", diagnostic.path.string(),
                diagnostic.message);
  }
  QMessageBox message{
      QMessageBox::Warning,
...
std::optional<std::string> TimelineTemplateController::PromptForTemplateName(
  ...
  SetInputDialogButtonAutomationIdentifiers(dialog, u"acceptTemplateNameButton",
                                            u"cancelTemplateNameButton");
  if (dialog.exec() != QDialog::Accepted) {
    return std::nullopt;
  }
...
void TimelineTemplateController::saveTemplate(void) {
  ...
  const auto name = PromptForTemplateName(tr("Save Filter and Export Template"),
                                          tr("Template name:"), {});
  if (!name.has_value()) {
    return;
  }
...
void TimelineTemplateController::SetEventProjection(
  ...
  if (!result.has_value()) {
    ShowCommandFailure(tr("Could Not Change Export Columns"), result.error());
    return;
  }
  RefreshTemplateState();
}
...
void TimelineTemplateController::ShowCommandFailure(
  ...
  switch (std::get<presentation::TimelineTemplateCommandError>(failure)) {
    case presentation::TimelineTemplateCommandError::kInvalidFilter:
      SPDLOG_ERROR(
          "Timeline template command was rejected because the "
          "filter is invalid");
      return;
...
void TimelineTemplateController::ShowServiceFailure(
  ...
  if (failure.filesystem_error) {
    details +=
        tr("\nSystem error: %1").arg(Utf8(failure.filesystem_error.message()));
  }
...
void TimelineTemplateController::updateTemplate(void) {
  if (!view_model_.IsFilterValid()) {
    ShowInvalidFilter(tr("Could Not Update Template"),
