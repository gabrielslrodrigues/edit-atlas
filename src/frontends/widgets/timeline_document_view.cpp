...
void TimelineDocumentView::BuildUi(void) {
  ...
  connect(timeline_filter_, &TimelineFilterWidget::saveTemplateRequested, this,
          &TimelineDocumentView::saveTemplateRequested);
...
void TimelineDocumentView::PopulateDiagnostics(
  ...
  for (const auto& diagnostic : diagnostics) {
    ...
    auto* item = new QTreeWidgetItem{
        diagnostics_tree_,
        {severity, line, presentation::diagnostic_text::Message(diagnostic)},
    };
    item->setToolTip(2, Utf8(diagnostic.code));
  }

  diagnostics_group_->setTitle(
      tr("Diagnostics (%1)").arg(static_cast<qulonglong>(diagnostics.size())));
