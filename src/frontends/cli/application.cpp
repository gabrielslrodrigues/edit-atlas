...
[[nodiscard]] CLI::App* ConfigureApplication(CLI::App& application,
  ...
  application.set_version_flag(
      "--version", std::string{"Edit Atlas "} + std::string{core::Version()});
  ...
  convert->add_option("input", options.input, "Input CMX 3600 EDL path")
      ->required();
  convert->add_option("output", options.output, "Output XLSX path")->required();
  convert
  ...
  convert
      ->add_option("--language", options.workbook_language, "Workbook language")
...
[[nodiscard]] core::Diagnostic FilesystemDiagnostic(
...
[[nodiscard]] std::string ImportFailureMessage(
...
[[nodiscard]] std::string ImportFailureCode(
    services::TimelineDocumentImportFailureKind kind) {
  return kind == services::TimelineDocumentImportFailureKind::kOpenFailed
             ? "cli.input.open_failed"
             : "cli.input.read_failed";
}

[[nodiscard]] std::string ExportFailureMessage(
...
[[nodiscard]] std::string ExportFailureCode(
    services::TimelineDocumentExportFailureKind kind) {
  switch (kind) {
    case services::TimelineDocumentExportFailureKind::kInvalidRequest:
...
[[nodiscard]] std::vector<core::MetadataEntry> ImportOptions(
...
[[nodiscard]] std::vector<core::MetadataEntry> ExportOptions(
    const Options& options) {
  return {
      core::MetadataEntry{
...
[[nodiscard]] ExitCode Convert(const Options& options, std::ostream& output,
  ...
  const services::TimelineRenderedVideoExportService export_service{*registry};
  auto export_result =
      export_service.Export(services::TimelineRenderedVideoExportRequest{
          ...
          .video_path = options.video.has_value()
    ...
    if (failure.diagnostics.empty()) {
      if (failure.document_export_failure.has_value()) {
        ...
        diagnostics.emplace_back(
            FilesystemDiagnostic(ExportFailureCode(document_failure.kind),
                                 message, document_failure.path));
      }
    } else {
...
ExitCode Run(std::span<const std::string_view> arguments, std::ostream& output,
  ...
  try {
    ...
    try {
      ...
      const auto cli11_exit_code = application.exit(parse_error, output, error);
      return cli11_exit_code == 0 ? ExitCode::kSuccess : ExitCode::kUsageError;
    }
    if (convert->parsed()) {
