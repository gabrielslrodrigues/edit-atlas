...
[[nodiscard]] bool RequiresInitialFrames(
    const TimelineDocumentExportRequest& request) noexcept {
  return std::ranges::contains(request.event_projection,
                               core::TimelineEventField::kInitialFrame);
}

[[nodiscard]] TimelineRenderedVideoExportFailure InspectionFailure(
    TimelineVideoInspectionFailure failure) {
...
[[nodiscard]] TimelineRenderedVideoExportFailure ExtractionFailure(
    const std::filesystem::path& path, TimelineFrameExtractionFailure failure) {
  return TimelineRenderedVideoExportFailure{
...
[[nodiscard]] TimelineRenderedVideoExportFailure DocumentExportFailure(
    TimelineDocumentExportFailure failure) {
...
TimelineRenderedVideoExportResult TimelineRenderedVideoExportService::Export(
  ...
  if (RequiresInitialFrames(request.document_export)) {
    ...
    if (stop_token.stop_requested()) {
      return std::unexpected(TimelineRenderedVideoExportFailure{
          .kind =
              TimelineRenderedVideoExportFailureKind::kFrameExtractionFailed,
    ...
    if (!inspection.has_value()) {
      return std::unexpected(InspectionFailure(std::move(inspection.error())));
    }
    ...
    if (!extraction.has_value()) {
      return std::unexpected(ExtractionFailure(*request.video_path,
                                               std::move(extraction.error())));
    }
    if (stop_token.stop_requested()) {
      return std::unexpected(TimelineRenderedVideoExportFailure{
          .kind =
    ...
    for (auto& frame : extraction->frames) {
      request.document_export.event_images.push_back(core::TimelineEventImage{
