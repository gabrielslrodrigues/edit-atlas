...
class TimelineDocumentExportService final {
 public:
  ...
  TimelineDocumentExportService(const TimelineDocumentExportService&) = delete;
  TimelineDocumentExportService& operator=(
      const TimelineDocumentExportService&) = delete;
  TimelineDocumentExportService(TimelineDocumentExportService&&) = delete;
  TimelineDocumentExportService& operator=(TimelineDocumentExportService&&) =
      delete;

  /// Exports and atomically commits a timeline according to \p request.
  ///
  /// \returns A receipt on success or a stage-specific failure.
  [[nodiscard]] TimelineDocumentExportResult Export(
