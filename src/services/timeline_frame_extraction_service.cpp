...
[[nodiscard]] std::optional<std::int64_t> AddFrames(
    std::int64_t frame, std::int64_t offset) noexcept {
  if ((offset > 0 &&
...
[[nodiscard]] bool ValidImage(
    const core::RgbImage& image,
    const TimelineFrameExtractionOptions& options) noexcept {
...
[[nodiscard]] std::optional<TimelineFrameExtractionFailure> ReportProgress(
    const TimelineFrameExtractionProgressCallback& callback,
...
TimelineFrameExtractionResult TimelineFrameExtractionService::Extract(
  ...
  if (!ValidOptions(options)) {
    return std::unexpected(
        Failure(timeline_frame_extraction_diagnostic_code::kInvalidOutputSize,
  ...
  for (const auto& [video_frame, event_indices] : events_by_frame) {
    ...
    while (!exact_frame.has_value()) {
      ...
      auto frame_result = inspection.decoder->ReadFrame(options.frame_output);
      if (!frame_result.has_value()) {
        auto decoder_failure = std::move(frame_result.error());
        return std::unexpected(
            Failure(timeline_frame_extraction_diagnostic_code::kDecodeFailed,
                    decoder_failure.message, std::move(decoder_failure)));
      }
      if (!frame_result->has_value()) {
        return std::unexpected(Failure(
            timeline_frame_extraction_diagnostic_code::kFrameUnavailable,
    ...
    if (!ValidImage(exact_frame->image, options)) {
      return std::unexpected(
          Failure(timeline_frame_extraction_diagnostic_code::kInvalidFrame,
                  "The decoded RGB frame image is invalid or exceeds the "
                  "requested output dimensions."));
    }
    auto image =
        std::make_shared<const core::RgbImage>(std::move(exact_frame->image));
