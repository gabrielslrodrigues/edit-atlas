...
[[nodiscard]] std::string ExpressionText(
    const TimelineTextFilterCondition& condition) {
  auto expression = condition.regular_expression
                        ? condition.text
                        : RE2::QuoteMeta(condition.text);
  if (condition.match_whole_word) {
    expression =
        R"((?:^|[^\p{L}\p{N}_])(?:)" + expression + R"()(?:$|[^\p{L}\p{N}_]))";
  }
  return expression;
}
...
[[nodiscard]] bool CommentsMatch(const core::EditEvent& event,
  ...
  return std::ranges::any_of(event.comments, [&](const core::Comment& comment) {
...
[[nodiscard]] bool EventMatches(const core::EditEvent& event,
  ...
  for (const auto& condition : conditions) {
    const auto matches = std::visit(
        [&event](const auto& value) { return ConditionMatches(event, value); },
        condition);
  ...
  return conditions.empty() || combination == TimelineFilterCombination::kAll ||
         matched_any;
}

}  // namespace

TimelineFilterResult FilterTimelineEvents(
    const core::TimelineDocument& document, const TimelineFilterQuery& query) {
  ...
  for (std::size_t index = 0; index < document.events.size(); ++index) {
    if (EventMatches(document.events[index], query.combination, *prepared)) {
...
core::TimelineDocument SelectTimelineEvents(
