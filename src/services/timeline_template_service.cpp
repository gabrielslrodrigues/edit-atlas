...
[[nodiscard]] TimelineTemplateFailure ServiceFailure(
    TimelineTemplateFailureKind kind, std::string message) {
...
[[nodiscard]] TimelineTemplateFailure ServiceFailure(
    const TimelineTemplateStoreFailure& failure) {
  return TimelineTemplateFailure{
...
TimelineTemplateLoadResult TimelineTemplateService::Load(void) {
  ...
  for (auto& value : result->templates) {
    const auto duplicate_name = std::ranges::any_of(
        templates_,
        [&value](const auto& existing) { return existing.name == value.name; });
    if (duplicate_name) {
      diagnostics.push_back({
...
std::span<const TimelineTemplate> TimelineTemplateService::Templates(
    void) const {
  return templates_;
}

const TimelineTemplate* TimelineTemplateService::Find(
    std::string_view identifier) const {
  const auto position =
      std::ranges::find(templates_, identifier, &TimelineTemplate::identifier);
...
TimelineTemplateResult TimelineTemplateService::Rename(
    std::string_view identifier, std::string name) {
  if (!loaded_) {
    return std::unexpected(
        ServiceFailure(TimelineTemplateFailureKind::kNotLoaded,
                       "The template catalog has not been loaded."));
...
TimelineTemplateResult TimelineTemplateService::Duplicate(
    std::string_view identifier, std::string name) {
  if (!loaded_) {
    return std::unexpected(
        ServiceFailure(TimelineTemplateFailureKind::kNotLoaded,
...
TimelineTemplateMutationResult TimelineTemplateService::Remove(
...
TimelineTemplateService::ValidateName(
  ...
  const auto duplicate = std::ranges::any_of(
      templates_, [name, excluded_identifier](const TimelineTemplate& value) {
        return value.identifier != excluded_identifier && value.name == name;
      });
  if (duplicate) {
    return std::unexpected(
...
TimelineTemplate* TimelineTemplateService::FindMutable(
    std::string_view identifier) {
  const auto position =
      std::ranges::find(templates_, identifier, &TimelineTemplate::identifier);
  return position == templates_.end() ? nullptr : &*position;
