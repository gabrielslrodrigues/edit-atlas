...
[[nodiscard]] voidpf ZCALLBACK OpenZipStream(voidpf opaque, const void*,
  ...
  try {
    ...
    stream->file.open(*path, std::ios::binary | std::ios::in | std::ios::out |
                                 std::ios::trunc);
...
// `long` matches minizip's seek64_file_func signature exactly.
[[nodiscard]] long ZCALLBACK SeekZipStream(  // NOLINT(runtime/int)
    voidpf, voidpf stream, ZPOS64_T offset, int origin) noexcept {
  auto* zip_stream = static_cast<ZipStream*>(stream);
  ...
  zip_stream->file.seekg(static_cast<std::streamoff>(position), std::ios::beg);
  if (!zip_stream->file) {
    zip_stream->error = true;
    return -1;
  }
...
[[nodiscard]] int ZCALLBACK CloseZipStream(voidpf, voidpf stream) noexcept {
  auto zip_stream = std::unique_ptr<ZipStream>{static_cast<ZipStream*>(stream)};
...
[[nodiscard]] zlib_filefunc64_def ZipFileFunctions(
...
ApplicationLogFiles(const SupportBundleRequest& request) {
  ...
  if (!std::filesystem::exists(request.log_directory, error)) {
    if (error) {
      return std::unexpected(Failure(request,
                                     SupportBundleFailureKind::kReadLogsFailed,
                                     error, error.message()));
    }
    return logs;
  }
...
[[nodiscard]] bool WriteZipEntry(zipFile archive, std::string_view name,
  ...
  if (zipOpenNewFileInZip64(
          archive, entry_name.c_str(), nullptr, nullptr, 0, nullptr, 0, nullptr,
          Z_DEFLATED, Z_DEFAULT_COMPRESSION, uses_zip64 ? 1 : 0) != ZIP_OK) {
    return false;
  }
...
[[nodiscard]] bool WriteZipEntry(zipFile archive, std::string_view name,
  ...
  return WriteZipEntry(
      archive, name, std::as_bytes(std::span{content.data(), content.size()}));
}
...
std::string FormatDiagnosticEnvironment(
...
CreateSupportBundleResult CreateSupportBundle(SupportBundleRequest request) {
  ...
  if (!transaction.has_value()) {
    const auto kind = transaction.error().kind ==
  ...
  if (write_failure.has_value() || close_result != ZIP_OK) {
    const auto kind =
        write_failure.value_or(SupportBundleFailureKind::kWriteBundleFailed);
  ...
  if (!commit_result.has_value()) {
    const auto kind = commit_result.error().kind ==
                              storage::LocalFileFailureKind::kDestinationExists
                          ? SupportBundleFailureKind::kDestinationExists
