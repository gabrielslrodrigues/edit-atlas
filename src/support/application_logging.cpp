...
[[nodiscard]] std::optional<std::size_t> RotationIndex(
...
class FilesystemRotatingSink final
 ...
 public:
  FilesystemRotatingSink(std::filesystem::path path,
      ...
      : path_(std::move(path)),
        maximum_file_size_(maximum_file_size),
 ...
 private:
  ...
  void Rotate(void) {
    ...
    if (error) {
      throw spdlog::spdlog_ex{"Could not remove an expired log rotation."};
    }
    ...
    for (auto index = maximum_files_ - 1; index > 1; --index) {
      ...
      if (!std::filesystem::exists(source, error)) {
        if (error) {
          throw spdlog::spdlog_ex{"Could not inspect a log rotation."};
        }
        continue;
      }
    ...
    if (std::filesystem::exists(path_, error)) {
      ...
      if (error) {
        throw spdlog::spdlog_ex{"Could not rotate the active application log."};
      }
    } else if (error) {
      throw spdlog::spdlog_ex{"Could not inspect the active application log."};
    }
    Open(true);
  }
...
[[nodiscard]] std::expected<void, std::string> PruneExpiredLogs(
...
[[nodiscard]] LoggingInitializationFailure Failure(
    const LoggingOptions& options, std::string message) {
...
InitializeApplicationLogging(const LoggingOptions& options) {
  try {
    if (options.directory.empty()) {
      ...
      return std::unexpected(Failure(options, "The log directory is empty."));
    }
    if (options.maximum_file_size == 0 ||
        options.maximum_file_size > (std::numeric_limits<std::size_t>::max)() ||
...
#if !defined(_WIN32)
    std::filesystem::permissions(options.directory,
