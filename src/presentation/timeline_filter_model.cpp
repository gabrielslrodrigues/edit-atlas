...
[[nodiscard]] services::TimelineTextFilterField TextField(
    TimelineFilterField field) {
  switch (field) {
    case TimelineFilterField::kEventIdentifier:
...
[[nodiscard]] services::TimelineTimecodeFilterField TimecodeField(
    TimelineFilterField field) {
  switch (field) {
    case TimelineFilterField::kSourceIn:
...
[[nodiscard]] TimelineFilterField PresentationField(
    services::TimelineTextFilterField field) {
  switch (field) {
...
[[nodiscard]] TimelineFilterField PresentationField(
    services::TimelineTimecodeFilterField field) {
  switch (field) {
...
[[nodiscard]] bool IsValidField(int value) {
  return value >= 0 && value < static_cast<int>(TimelineFilterField::kCount);
}
...
void TimelineFilterModel::SetQuery(const services::TimelineFilterQuery& query) {
  const auto previous_query = Query();
  beginResetModel();
  ...
  for (const auto& condition : query.conditions) {
    ...
    std::visit(
        [&row](const auto& value) {
          ...
          if constexpr (std::same_as<Condition,
            ...
            row.text =
                value.frames.has_value()
                    ? QString::number(static_cast<qlonglong>(*value.frames))
                    : QString{};
          }
        },
        condition);
  ...
  for (const auto& row : rows_) {
    switch (EditorForField(row.field)) {
      ...
      case TimelineFilterEditor::kEditType:
        query.conditions.emplace_back(services::TimelineEditTypeFilterCondition{
      ...
      case TimelineFilterEditor::kTimecode:
        ...
        query.conditions.emplace_back(services::TimelineTimecodeFilterCondition{
      ...
      case TimelineFilterEditor::kDuration: {
        ...
        query.conditions.emplace_back(services::TimelineDurationFilterCondition{
            .frames =
                std::optional<std::int64_t>{static_cast<std::int64_t>(frames)},
        });
        break;
      }
...
void TimelineFilterModel::SetCombination(int combination) {
  if (combination <
          static_cast<int>(services::TimelineFilterCombination::kAll) ||
      combination >
          static_cast<int>(services::TimelineFilterCombination::kAny) ||
      static_cast<int>(combination_) == combination) {
    return;
  }
  combination_ = static_cast<services::TimelineFilterCombination>(combination);
  emit queryChanged();
}
...
QStringList TimelineFilterModel::FieldNames(void) const {
  return {
      tr("Event"),           tr("Reel"),
      tr("Track type"),      tr("Track ID"),
      tr("Edit type"),       tr("Clip"),
      tr("Source In"),       tr("Source Out"),
      tr("Record In"),       tr("Record Out"),
      tr("Duration frames"), tr("Comments"),
  };
}
