...
TimelineDocumentViewModel::TimelineDocumentViewModel(
  ...
  connect(&workflow_, &TimelineDocumentWorkflow::frameExtractionProgressChanged,
...
TimelineDocumentCommandResult TimelineDocumentViewModel::SetEventProjection(
  ...
  if (!core::IsValidTimelineEventProjection(projection)) {
    return std::unexpected{TimelineDocumentCommandError::kInvalidProjection};
  }
...
TimelineDocumentCommandResult TimelineDocumentViewModel::Export(
  ...
  services::TimelineDocumentExportRequest document_export{
      ...
      .timeline = services::SelectTimelineEvents(*document_, event_selection_),
...
TimelineDocumentState TimelineDocumentViewModel::DocumentState(
    void) const noexcept {
  return document_state_;
}

TimelineExportState TimelineDocumentViewModel::ExportState(
    void) const noexcept {
...
bool TimelineDocumentViewModel::IsBusy(void) const noexcept {
  return document_state_ == TimelineDocumentState::kImporting ||
         export_state_ == TimelineExportState::kExporting || workflow_.IsBusy();
}
...
const std::filesystem::path& TimelineDocumentViewModel::SourcePath(
    void) const noexcept {
  return source_path_;
}

const core::TimelineDocument* TimelineDocumentViewModel::Document(
    void) const noexcept {
  return document_.has_value() ? &*document_ : nullptr;
}

std::span<const core::Diagnostic> TimelineDocumentViewModel::ImportDiagnostics(
    void) const noexcept {
...
const services::TimelineFilterQuery& TimelineDocumentViewModel::FilterQuery(
    void) const noexcept {
  return filter_query_;
}

std::span<const std::size_t> TimelineDocumentViewModel::EventSelection(
    void) const noexcept {
  return event_selection_;
}

const services::TimelineFilterError* TimelineDocumentViewModel::FilterError(
    void) const noexcept {
...
const TimelineEventModel& TimelineDocumentViewModel::EventModel(
    void) const noexcept {
  return event_model_;
}
...
const DiagnosticModel& TimelineDocumentViewModel::DiagnosticsModel(
    void) const noexcept {
  return diagnostics_model_;
}
...
void TimelineDocumentViewModel::handleImportFinished(void) {
  ...
  SPDLOG_INFO(
      "Timeline import completed with {} event(s) and {} "
      "diagnostic(s)",
...
void TimelineDocumentViewModel::handleExportFinished(void) {
  ...
  if (!export_result_->has_value()) {
    SPDLOG_ERROR(
        "Rendered-video spreadsheet export failed at stage {} "
        "with {} diagnostic(s)",
