...
[[nodiscard]] std::vector<core::TimelineEventField> DefaultEventProjection(
    void) {
  const auto projection = core::DefaultTimelineEventProjection();
  return {projection.begin(), projection.end()};
}

[[nodiscard]] TimelineTemplateCommandFailure CommandFailure(
    TimelineTemplateCommandError error) {
  return error;
}

[[nodiscard]] TimelineTemplateCommandFailure ServiceFailure(
    services::TimelineTemplateFailure failure) {
  return failure;
}
...
TimelineTemplateCommandResult TimelineTemplateViewModel::Create(
    std::string name) {
...
TimelineTemplateCommandResult TimelineTemplateViewModel::RenameActive(
    std::string name) {
  const auto active = RequireActive();
  if (!active.has_value()) {
    return active;
...
TimelineTemplateCommandResult TimelineTemplateViewModel::DuplicateActive(
    std::string name) {
  const auto active = RequireActive();
  if (!active.has_value()) {
    return active;
...
const TimelineTemplateModel& TimelineTemplateViewModel::TemplateModel(
    void) const noexcept {
  return template_model_;
}

const std::optional<std::string>& TimelineTemplateViewModel::ActiveIdentifier(
    void) const noexcept {
  return active_identifier_;
}

const services::TimelineTemplate* TimelineTemplateViewModel::ActiveTemplate(
    void) const noexcept {
  return active_identifier_.has_value() ? service_.Find(*active_identifier_)
                                        : nullptr;
}

const services::TimelineFilterQuery& TimelineTemplateViewModel::FilterQuery(
    void) const noexcept {
  return filter_;
}
...
TimelineTemplateCommandResult TimelineTemplateViewModel::ValidateWritableState(
    void) const {
  if (!filter_valid_) {
    return std::unexpected{
...
TimelineTemplateCommandResult TimelineTemplateViewModel::RequireActive(
    void) const {
