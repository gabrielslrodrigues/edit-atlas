...
$message = "vcpkg binary packages: $restoredCount restored, " `
  + "$($built.Count) built"
Write-Host $message
if ($built.Count -gt 0) {
...
if ($env:GITHUB_OUTPUT) {
  Add-Content -Path $env:GITHUB_OUTPUT `
    -Value "restored-package-count=$restoredCount"
  Add-Content -Path $env:GITHUB_OUTPUT `
    -Value "built-package-count=$($built.Count)"
}
