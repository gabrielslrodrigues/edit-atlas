...
# Validate the complete release asset set and its corresponding sources.

# Record an invalid asset while allowing the rest of the inventory to report.
# Globals: failures (updated in the active main invocation).
# Arguments: diagnostic text. Outputs: diagnostic to stderr.
fail() {
  echo "  MISSING OR INVALID: $*" >&2
  failures=$((failures + 1))
}

# Check required package names, source archives, notices, and checksums.
# Arguments: release directory, vX.Y.Z tag.
# Outputs: asset inventory to stdout; invalid assets to stderr.
# Returns: 2 for invalid arguments, 1 when any asset fails validation.
main() {
  local archive
  local expected
  local failures
  ...
  (
    cd -- "$release_dir" \
      && find . -maxdepth 1 -type f -printf '  %-48f %10s bytes\n' \
      | sort
  )
  echo
