...
[[nodiscard]] std::filesystem::path FixturePath(std::string_view name) {
  return std::filesystem::path{EDIT_ATLAS_INTEGRATION_FIXTURE_DIRECTORY} / name;
}
...
TEST(TimelineDocumentViewModelTest, ImportsFiltersAndClearsTimelineState) {
  ...
  EXPECT_EQ(clear_while_importing.error(), TimelineDocumentCommandError::kBusy);
  imported.clear();
...
TEST(TimelineDocumentViewModelTest, ExportsTheFilteredProjectedTimeline) {
  ...
  QSignalSpy exported{&view_model, &TimelineDocumentViewModel::exportFinished};
...
TEST(TimelineDocumentViewModelTest, RetainsImportFailureForTheFrontend) {
  ...
  ASSERT_TRUE(registry.has_value());
  TimelineDocumentViewModel view_model{*registry};
  QSignalSpy changed{&view_model, &TimelineDocumentViewModel::documentChanged};
  ...
  EXPECT_EQ(view_model.DiagnosticsModel().rowCount(),
            static_cast<int>(view_model.ImportFailure()->diagnostics.size()));
}
