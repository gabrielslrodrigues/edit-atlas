...
[[nodiscard]] TimelineTemplateCommandError CommandError(
    const TimelineTemplateCommandResult& result) {
...
TEST(TimelineTemplateViewModelTest, CreatesUpdatesAndRestoresTemplateState) {
  ...
  QSignalSpy modified{&view_model, &TimelineTemplateViewModel::modifiedChanged};
  ...
  EXPECT_TRUE(
      std::ranges::equal(view_model.EventProjection(),
...
TEST(TimelineTemplateViewModelTest, ReloadsPersistedTemplateState) {
  ...
  ASSERT_TRUE(original.SetEventProjection({core::TimelineEventField::kRecordIn})
                  .has_value());
