...
class BlockingImporter final : public core::Importer {
 public:
  ...
  [[nodiscard]] core::ProbeConfidence Probe(
      const core::ImportRequest&) const override {
...
TEST(TimelineDocumentWorkflowTest,
  ...
  const auto shared_strings = ReadZipEntry(destination, "xl/sharedStrings.xml");
...
TEST(TimelineDocumentWorkflowTest, RecoversAfterAnExportFailure) {
  ...
  const auto imported = importer.Import(services::TimelineDocumentImportRequest{
      .path = FixturePath("mixed_tracks.edl"),
      .format_identifier = {},
      .options =
          {
