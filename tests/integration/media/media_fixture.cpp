...
[[nodiscard]] std::expected<void, std::string> WriteAvailablePackets(
  ...
  for (int row = 0; row < frame.height; ++row) {
    for (int column = 0; column < frame.width; ++column) {
      frame.data[0][row * frame.linesize[0] + column] =
          static_cast<std::uint8_t>((column + row + frame_index * 7) % 256);
    }
  }
...
std::expected<void, std::string> WriteVideoFixture(
  ...
  const auto allocate_result = avformat_alloc_output_context2(
      &raw_format_context, nullptr, container_text.c_str(), path_text.c_str());
  ...
  codec_context->time_base =
      AVRational{options.frame_rate_denominator, options.frame_rate_numerator};
  codec_context->framerate =
      AVRational{options.frame_rate_numerator, options.frame_rate_denominator};
  ...
  if (parameters_result < 0) {
    return std::unexpected(
        Failure("could not configure the fixture stream", parameters_result));
  }
  ...
  if (options.starting_timecode.has_value()) {
    ...
    if (av_dict_set(&format_context->metadata, "timecode", timecode, 0) < 0 ||
        av_dict_set(&stream->metadata, "timecode", timecode, 0) < 0) {
      return std::unexpected("could not set the fixture starting timecode");
    }
  }
  ...
  if (frame_buffer_result < 0) {
    return std::unexpected(
        Failure("could not allocate the fixture frame", frame_buffer_result));
  }
  ...
  auto write_result =
      WriteAvailablePackets(*format_context, *codec_context, *stream, *packet);
  if (!write_result.has_value()) {
    return write_result;
  }
