...
[[nodiscard]] bool WriteFixture(const std::filesystem::path& directory,
  ...
  const auto result = WriteVideoFixture(directory / filename, "mov", options);
  if (!result.has_value()) {
    std::cerr << "Could not generate " << filename << ": " << result.error()
              << '\n';
    return false;
  }
  return true;
...
[[nodiscard]] bool WriteCancellationEdl(const std::filesystem::path& directory,
                                        std::int32_t frame_count) {
  ...
  for (std::int32_t frame = 0; frame < frame_count; ++frame) {
    output << std::setfill('0') << std::setw(3) << frame + 1
           ...
           << Timecode(frame + 1) << ' ' << Timecode(kOneHourAt24Fps + frame)
