...
TEST(ApplicationStateTest, RoutesEveryPersistentStoreBelowTheOverride) {
  ...
  EXPECT_TRUE(
      IsBelowDirectory(FilesystemPath(settings.fileName()), root / "settings"));
}
...
TEST(ApplicationStateTest, IsolatesRecentFilesAndTimelineTemplates) {
  ...
  EXPECT_TRUE(settings.value(QStringLiteral("files/rememberRecent")).toBool());
  ...
  const auto created =
      templates.Create("Integration template", services::TimelineFilterQuery{},
