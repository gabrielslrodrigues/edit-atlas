...
void ExpectUniqueIdentifiers(QWidget& root) {
  ...
  for (const auto* widget : widgets) {
    ...
    EXPECT_FALSE(identifiers.contains(identifier)) << identifier.toStdString();
    identifiers.insert(identifier);
  }
  for (const auto* action : root.findChildren<QAction*>()) {
    const auto identifier = action->property("accessibleIdentifier").toString();
    if (identifier.isEmpty()) {
      continue;
    }
    EXPECT_FALSE(identifiers.contains(identifier)) << identifier.toStdString();
...
TEST(AccessibilityContractTest,
  ...
  for (const auto* identifier : action_identifiers) {
    auto* action = window.findChild<QAction*>(QString::fromLatin1(identifier));
  ...
  auto* remember =
      window.findChild<QAction*>(QStringLiteral("rememberRecentFilesAction"));
...
TEST(AccessibilityContractTest, CoversSpreadsheetOptionsAndProjectionControls) {
  ...
  EXPECT_NE(std::ranges::find(changed, core::TimelineEventField::kInitialFrame),
            changed.end());
...
TEST(AccessibilityContractTest, CoversTemplateProjectionDialogControls) {
  ...
  constexpr std::array required_identifiers{
      "eventProjectionDialog",  "eventProjectionWidget", "eventColumnsList",
      "moveColumnUpButton",     "moveColumnDownButton",  "saveProjectionButton",
...
TEST(AccessibilityContractTest, PersistsLanguageWithoutChangingIdentifiers) {
  ...
  auto* open = window.findChild<QAction*>(QStringLiteral("openDocumentAction"));
...
TEST(AccessibilityContractTest, AcceptsOnlyLocalFilesForDragAndDrop) {
  ...
  remote_data.setUrls({QUrl{QStringLiteral("https://example.invalid/a.edl")}});
