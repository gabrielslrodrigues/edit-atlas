...
# Provision a container and run the packaged Linux E2E suite.
...
# Run the installed package in the pinned container desktop environment.
# Arguments: package and fixture options, then optional pytest arguments.
# Outputs: provisioning and suite output; configuration errors to stderr.
# Returns: 2 for invalid inputs, 127 for missing tools, or the suite status.
main() {
  local artifact_directory
  local base_image
  local build_arguments
  local container_arguments
  local container_fixture_directory
  ...
  local image
  local media_fixture_directory
  local option
  local package_path
  local provision_arguments
  local pytest_arguments
  ...
  if [[ -n "${fixture_generator}" ]]; then
    ...
    # so running it in the image requires the host and image to share an ABI.
    # A host with a newer glibc than the pinned Ubuntu base cannot satisfy
    ...
    printf 'Media-fixture directory does not exist: %s\n' \
      "${media_fixture_directory}" >&2
    exit 2
  fi
