...
# Prepare the container desktop and run tests against an installed package.
...
# Install the supplied DEB and run its desktop suite as the invoking host user.
# Arguments: repository, artifacts, package, fixtures, home, UID and GID
#   options, followed by optional pytest arguments.
# Outputs: installation and test output; prerequisite failures to stderr.
# Returns: 2 for invalid inputs, otherwise the failed command or suite status.
main() {
  local artifact_root
  local e2e_root
