...
[[nodiscard]] core::ImportRequest Request(
    const std::string& text, std::vector<core::MetadataEntry> options = {}) {
...
[[nodiscard]] bool HasMetadata(const std::vector<core::MetadataEntry>& metadata,
                               std::string_view key) {
  return std::ranges::any_of(metadata, [key](const core::MetadataEntry& entry) {
...
TEST(Cmx3600ImporterTest, DescribesAndRecognizesTheFormat) {
  ...
  EXPECT_EQ(importer.descriptor().extensions, std::vector<std::string>{"edl"});
...
TEST(Cmx3600ImporterTest, SupportsCrLfUtf8BomAndDropFrame) {
  ...
  EXPECT_EQ(result.document->events.front().source_range.DurationInFrames(), 1);
}

TEST(Cmx3600ImporterTest, SupportsUtf16LittleEndianInput) {
  const Cmx3600Importer importer;
  const auto text = Utf16Le(
      "TITLE: UTF16\nFCM: NON-DROP FRAME\n"
