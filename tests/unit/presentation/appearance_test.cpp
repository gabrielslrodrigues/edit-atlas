...
TEST_F(AppearanceTest, PresentsLightWhenNoPlatformSchemeIsAvailable) {
  ...
  const AppearanceController controller;
  EXPECT_EQ(controller.ResolvedAppearanceValue(), ResolvedAppearance::kLight);
}
...
TEST_F(AppearanceTest, PalettesDifferAndEveryColorIsComplete) {
  ...
  for (const auto* palette : {&dark, &light}) {
    for (const auto& color : {palette->accent,
                              palette->accent_hovered,
                              palette->accent_pressed,
                              palette->on_accent,
                              palette->window,
                              palette->surface,
                              palette->surface_alternate,
                              palette->control,
                              ...
                              palette->warning,
                              palette->danger,
                              palette->tooltip_surface,
                              palette->tooltip_text}) {
      EXPECT_EQ(color.size(), 7) << color.toStdString();
      EXPECT_TRUE(color.startsWith(QLatin1Char('#'))) << color.toStdString();
    }
  }
}
