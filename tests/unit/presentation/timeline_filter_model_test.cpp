...
TEST(TimelineFilterModelTest, EditsAStablePresentationIndependentQuery) {
  ...
  EXPECT_TRUE(model.setData(first, static_cast<int>(TimelineFilterField::kClip),
                            TimelineFilterModel::kFieldRole));
  EXPECT_TRUE(model.setData(first, QStringLiteral("Opening"),
                            TimelineFilterModel::kTextRole));
  EXPECT_TRUE(model.setData(first, true, TimelineFilterModel::kMatchCaseRole));
  EXPECT_TRUE(
      model.setData(first, true, TimelineFilterModel::kMatchWholeWordRole));
...
TEST(TimelineFilterModelTest, RoundTripsEveryTypedCondition) {
      ...
      .conditions =
          {
              services::TimelineTextFilterCondition{
                  .field = services::TimelineTextFilterField::kTrackIdentifier,
                  .text = "V2",
                  .match_case = true,
  ...
  EXPECT_EQ(
      model.data(model.index(1, 0), TimelineFilterModel::kEditorRole).toInt(),
  ...
  EXPECT_EQ(
      model.data(model.index(3, 0), TimelineFilterModel::kTextRole).toString(),
      QStringLiteral("01:02:03:04"));
  EXPECT_EQ(
      model.data(model.index(4, 0), TimelineFilterModel::kTextRole).toString(),
      QStringLiteral("48"));
}
