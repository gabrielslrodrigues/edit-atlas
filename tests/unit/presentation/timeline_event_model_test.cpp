...
TEST(TimelineEventModelTest, PresentsTimelineDocumentRows) {
  ...
  EXPECT_EQ(
      model.data(model.index(0, 7), TimelineEventModel::kSortRole).toLongLong(),
      86'400);
  EXPECT_EQ(
      model.data(model.index(0, 9), TimelineEventModel::kSortRole).toLongLong(),
      24);
