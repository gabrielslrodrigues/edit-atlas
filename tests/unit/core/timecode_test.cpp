...
[[nodiscard]] Timecode MakeTimecode(
  ...
  return Timecode::Create(hours, minutes, seconds, frames, rate, mode).value();
}
...
TEST(TimecodeTest, RejectsInvalidComponents) {
  ...
  EXPECT_EQ(
      Timecode::Create(24, 0, 0, 0, rate, TimecodeMode::kNonDropFrame).error(),
      TimecodeError::kHourOutOfRange);
  EXPECT_EQ(
      Timecode::Create(0, 60, 0, 0, rate, TimecodeMode::kNonDropFrame).error(),
      TimecodeError::kMinuteOutOfRange);
  EXPECT_EQ(
      Timecode::Create(0, 0, 60, 0, rate, TimecodeMode::kNonDropFrame).error(),
      TimecodeError::kSecondOutOfRange);
  EXPECT_EQ(
      Timecode::Create(0, 0, 0, 25, rate, TimecodeMode::kNonDropFrame).error(),
