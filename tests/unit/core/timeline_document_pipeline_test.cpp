...
TEST(TimelineDocumentPipelineTest, UsesAnExplicitImporterWithoutProbing) {
  ...
  const auto result = pipeline.Import(Request(), std::string_view{"cmx-3600"});
...
TEST(TimelineDocumentPipelineTest, UsesExtensionToBreakEqualProbeConfidence) {
  FormatRegistry registry;
  auto edl = std::make_unique<test::StubImporter>(Descriptor("cmx-3600", "edl"),
                                                  ProbeConfidence::kHigh);
...
TEST(TimelineDocumentPipelineTest,
  ...
  EXPECT_TRUE(HasDiagnostic(
      result.diagnostics, pipeline_diagnostic_code::kImportProducedNoDocument));
}
...
TEST(TimelineDocumentPipelineTest,
  ...
  EXPECT_FALSE(result.artifact.has_value());
  EXPECT_TRUE(HasDiagnostic(
      result.diagnostics, pipeline_diagnostic_code::kExportProducedNoArtifact));
}
