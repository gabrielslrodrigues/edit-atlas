...
TEST(ApplicationTest, LoadsBrazilianPortugueseTranslations) {
  ...
  EXPECT_EQ(
      QCoreApplication::translate(
          "edit_atlas::frontends::widgets::EventProjectionWidget", "Duration"),
  ...
  const core::Diagnostic diagnostic{
      ...
      .code = std::string{formats::cmx3600::diagnostic_code::kMissingFrameRate},
...
TEST(ApplicationStyleTest, StyleSheetsDifferBetweenAppearances) {
  const auto light =
      LoadApplicationStyleSheet(presentation::AppearancePaletteFor(
