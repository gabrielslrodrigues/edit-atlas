...
[[nodiscard]] core::TimelineDocument Timeline(
    const core::FrameRate& rate = Rate(),
    core::TimecodeMode mode = core::TimecodeMode::kNonDropFrame,
    std::int64_t start_frame = 90'000, std::int64_t duration = 3) {
  auto start = core::Timecode::FromFrameCount(start_frame, rate, mode).value();
  auto end = core::Timecode::FromFrameCount(start_frame + duration, rate, mode)
                 .value();
              ...
              core::EditEvent{
                  ...
                  .source_range =
                      core::TimecodeRange::Create(std::move(source_start),
                                                  std::move(source_end))
                          .value(),
                  .record_range = core::TimecodeRange::Create(std::move(start),
                                                              std::move(end))
                                      .value(),
                  .comments = {},
                  .metadata = {},
...
[[nodiscard]] media::VideoMediaInfo MediaInformation(
    std::string timecode = "01:00:00:00",
...
TEST(TimelineVideoInspectionServiceTest, RejectsEmptyTimeline) {
  ...
  ExpectOnlyDiagnostic(result, timeline_video_diagnostic_code::kEmptyTimeline);
}
