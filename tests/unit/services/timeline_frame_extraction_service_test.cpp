...
[[nodiscard]] media::DecodedVideoFrame Frame(
    std::int64_t frame_index, const media::VideoFrameOutputOptions& options) {
  ...
  if (options.size_limit.has_value() &&
    ...
    const auto width_limited =
        static_cast<std::int64_t>(options.size_limit->maximum_width) * height <=
        static_cast<std::int64_t>(options.size_limit->maximum_height) * width;
    if (width_limited) {
      height = std::max<std::int32_t>(
          1,
          static_cast<std::int32_t>(static_cast<std::int64_t>(height) *
                                    options.size_limit->maximum_width / width));
      width = options.size_limit->maximum_width;
    } else {
      width = std::max<std::int32_t>(
          1, static_cast<std::int32_t>(static_cast<std::int64_t>(width) *
                                       options.size_limit->maximum_height /
                                       height));
      height = options.size_limit->maximum_height;
    }
  }
...
class FakeVideoDecoder final : public media::VideoDecoder {
 public:
  ...
  [[nodiscard]] const media::VideoMediaInfo& Information(
      void) const noexcept override {
    return information_;
  }

  [[nodiscard]] std::expected<void, media::VideoDecoderFailure> Seek(
      std::int64_t timestamp) override {
    return SeekToFrame(timestamp);
  }

  [[nodiscard]] std::expected<void, media::VideoDecoderFailure> SeekToFrame(
...
[[nodiscard]] core::EditEvent Event(std::string identifier,
  ...
  auto source_start =
      core::Timecode::FromFrameCount(0, rate, core::TimecodeMode::kNonDropFrame)
          .value();
  auto source_end =
      core::Timecode::FromFrameCount(1, rate, core::TimecodeMode::kNonDropFrame)
          .value();
  auto record_start = core::Timecode::FromFrameCount(
                          record_frame, rate, core::TimecodeMode::kNonDropFrame)
                          .value();
  auto record_end =
...
[[nodiscard]] TimelineVideoInspectionReceipt Inspection(
    const std::shared_ptr<FakeDecoderState>& state) {
  const auto rate = Rate();
...
TEST(TimelineFrameExtractionServiceTest, PreservesDecoderSeekFailure) {
  ...
  ExpectOnlyDiagnostic(result,
                       timeline_frame_extraction_diagnostic_code::kSeekFailed);
...
TEST(TimelineFrameExtractionServiceTest, RejectsThrowingProgressCallback) {
  ...
  const auto result =
      service.Extract(Timeline(), Inspection(state), {}, {},
