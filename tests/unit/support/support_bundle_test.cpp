...
[[nodiscard]] std::vector<std::string> ZipEntryNames(
  ...
  if (unzGoToFirstFile(archive) == UNZ_OK) {
    do {
      ...
      // `unsigned long` matches minizip's uLong parameter type exactly.
      if (unzGetCurrentFileInfo64(
              archive, nullptr, name.data(),
              static_cast<unsigned long>(name.size()),  // NOLINT(runtime/int)
...
TEST(SupportBundleTest, FormatsOnlyDocumentedEnvironmentMetadata) {
  ...
  EXPECT_NE(summary.find("Architecture: test-architecture"), std::string::npos);
  ...
  EXPECT_NE(summary.find("does not automatically include"), std::string::npos);
}
...
TEST(SupportBundleTest, PreservesExistingBundleWithoutPermission) {
  ...
  EXPECT_EQ(result.error().kind, SupportBundleFailureKind::kDestinationExists);
